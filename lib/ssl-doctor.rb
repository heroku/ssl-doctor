...
[[SSLTool::KeyHelper::KeyNotPresentError, 400, "No valid, non-passphrase-protected keys given."],
  ...
  [SSLTool::ChainResolution::TooManyHeadsChainResolutionError, 422, "More than one domain name certificate given."]].each { |err, code, msg| error(err) { error(code, msg) } }

def respond(data)
  if data.respond_to?(:to_pem) then content_type :text; data.to_pem
  elsif data.is_a?(Hash) then content_type :json; Yajl::Encoder.encode(data)
